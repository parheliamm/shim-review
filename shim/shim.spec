...
%ifarch %{ix86}
%define shim_provide_arch ia32
%else
%ifarch x86_64
%define shim_provide_arch x64
%else
%ifarch aarch64
%define shim_provide_arch aa64
%else
%ifarch %{arm}
%define shim_provide_arch arm
%else
%define shim_provide_arch %{_target_cpu}-efi
%endif
%endif
%endif
%endif
...
Source14:       signature-sel.x86_64.asc
Source15:       signature-ycl.x86_64.asc
Source16:       signature-sel.aarch64.asc
Source17:       signature-ycl.aarch64.asc
Source20:       YUNCHE-UEFI-CA-Certificate.crt
...
Source54:       vendor-dbx-suseeuler.bin
...
Patch6:         backport-shim-Enable-the-NX-compatibility-flag-by-default.patch
BuildRequires:  dos2unix
BuildRequires:  nss-tools
BuildRequires:  openssl >= 0.9.8
BuildRequires:  pesign
BuildRequires:  pesign-obs-integration
%if 0%{?suse_version} > 1320 || 0%{?_sel_version_code} >= 200 ||  0%{?_ycl_version_code} >= 200
...
Obsoletes:      %{name}-%{shim_provide_arch} < %{version}-%{release}
...
%endif
%if 0%{?is_opensuse} == 0 && 0%{?sle_version} > 0
distro_id="sle"
distro_name="SUSE Linux Enterprise"
%endif
%if 0%{?_sel_version_code} >= 200
distro_id="sel"
distro_name="%{distribution}"
distro_url="mailto:euler-security@suse.com"
%endif
%if 0%{?_ycl_version_code} >= 200
distro_id="ycl"
distro_name="%{distribution}"
distro_url="mailto:security@yuncheos.com"
%endif
distro_sbat=1

sbat="shim.${distro_id},${distro_sbat},${distro_name},%{name},%{version}-%{release},${distro_url}"
...
default=''
%if 0%{?_sel_version_code} >= 200
suffixes=(sel opensuse sles)
%else
%if 0%{?_ycl_version_code} >= 200
suffixes=(ycl opensuse sles)
%endif
%endif
...
%if 0%{?_sel_version_code} >= 200
    selsubject=$(openssl x509 -in %{SOURCE20} -noout -subject_hash)
%else
%if 0%{?_ycl_version_code} >= 200
    yclsubject=$(openssl x509 -in %{SOURCE20} -noout -subject_hash)
%endif
%endif
    if test "$prjsubject" = "$opensusesubject" ; then
	suffixes=(opensuse)
    elif test "$prjsubject" = "$slessubject" ; then
	suffixes=(sles)
%if 0%{?_sel_version_code} >= 200
    elif test "$prjsubject" = "$selsubject" ; then
	suffixes=(sel)
%else
%if 0%{?_ycl_version_code} >= 200
    elif test "$prjsubject" = "$yclsubject" ; then
	suffixes=(ycl)
%endif
%endif
    elif test "$prjsubject" = "$prjissuer" ; then
%if 0%{?_sel_version_code} >= 200
	suffixes=(devel opensuse sles sel)
%else
%if 0%{?_ycl_version_code} >= 200
	suffixes=(devel opensuse sles ycl)
%endif
%endif
    fi
fi
...
%endif
%if 0%{?_sel_version_code} >= 200
    elif test "$suffix" = "sel"; then
%else
%if 0%{?_ycl_version_code} >= 200
    elif test "$suffix" = "ycl"; then
%endif
%endif
	cert=%{SOURCE20}
	verify='Yunche Secure Boot Signing'
	vendor_dbx=%{SOURCE54}
%ifarch x86_64
%if 0%{?_sel_version_code} >= 200
	signature=%{SOURCE14}
